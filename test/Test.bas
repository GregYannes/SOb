...
Public Sub Test()
	...
	Dim dix As Object: Set dix = Test_Dix.New_Dix()
	...
	Debug.Print "IsObj(dix, """ & Test_Dix.DIX_CLS & """) = " & SOb.IsObj(dix, Test_Dix.DIX_CLS)
	...
	' ### Fields 1 ###
	SOb.Obj_Field(dix, Test_Dix.Dix_Field.Count) = 42
	
	SOb.Obj_FieldKey copy, Test_Dix.Dix_Field.Count
	Debug.Print "Obj_FieldKey(Dix_Field.Count) = """ & copy & """"
	Debug.Print "Obj_HasField(dix, Dix_Field.Count) = " & SOb.Obj_HasField(dix, Test_Dix.Dix_Field.Count)
	Debug.Print "Obj_Field(dix, Dix_Field.Count) = " & SOb.Obj_Field(dix, Test_Dix.Dix_Field.Count)
	...
	' ### Typology 2 ###
	Debug.Print "IsDix(dix) = " & Test_Dix.IsDix(dix)
	
	
	' ### Fields 2 ###
	Test_Dix.Dix_Count(dix) = 7
	Debug.Print "Dix_Count(dix) = " & Test_Dix.Dix_Count(dix)
...
Private Sub Test_Print(ByRef dix As Object)
	...
	Dim summary As String: summary = "" & Test_Dix.Dix_Count(dix)
	Dim detail As String: detail = "" & SOb.Obj_FormatFields0( _
		"Keys", "Collection[" & Test_Dix.Dix_Keys(dix).Count & "]", _
		"Items", "Collection[" & Test_Dix.Dix_Items(dix).Count & "]", _
		"Count", Test_Dix.Dix_Count(dix) _
	)
